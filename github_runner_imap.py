...
import order_engine as oe  # ваш двигун
...
def fetch_latest_attachment():
    ...
    typ,_ = M.select(IMAP_FOLDER);  assert typ=="OK", f"IMAP: неможливо відкрити {IMAP_FOLDER}"
    typ,data = M.search(None, "ALL"); assert typ=="OK", "IMAP search failed"
    ...
    for num in ids[:80]:
        ...
        for part in msg.walk():
            ...
            raw = part.get_filename();  if not raw: continue
...
def top3(df):
    ...
    return ", ".join(f"{r.product_name} — {int(r.order_qty) if float(r.order_qty).is_integer() else r.order_qty} {r.unit}"
...
def main():
    ...
    if not po_a.empty: po_a.to_csv(os.path.join(oe.OUT_DIR,"PO_Боголюбова.csv"), index=False, encoding="utf-8-sig")
    if not po_b.empty: po_b.to_csv(os.path.join(oe.OUT_DIR,"PO_Європейська_31а.csv"), index=False, encoding="utf-8-sig")
    summary = f"Магазин {oe.STORE_A_NAME}: {len(po_a)} позицій. Топ: {top3(po_a)}\n" \
    ...
    if not po_a.empty: tg_send_document(os.path.join(oe.OUT_DIR,"PO_Боголюбова.csv"), caption="Замовлення: <code>PO_Боголюбова.csv</code>")
    if not po_b.empty: tg_send_document(os.path.join(oe.OUT_DIR,"PO_Європейська_31а.csv"), caption="Замовлення: <code>PO_Європейська_31а.csv</code>")
